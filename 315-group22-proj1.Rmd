...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(countrycode)
top10.airlines <- names(sort(table(airline$airline_name), decreasing = TRUE)[1:10])
...
airline.bar$region <- factor(countrycode(sourcevar = airline.bar[["author_country"]],
                                   origin = "country.name",
                                   destination = "region"))
airline.bar <- airline.bar[airline.bar$airline_name %in% top10.airlines, ]

ggplot(airline.bar, aes(x = airline_name, fill = continent)) +
  geom_bar() +
  labs(title = "Airline review counts by author continent",
       x = "Airline") +
  project1_theme +
